...
The [context window](https://opencode.ai/docs/context/) is a public good. Your Skill shares the context window with everything else the agent needs to know, including:
...
The most effective Skill development process involves the agent itself. Work with one instance of the agent ("the agent A") to create a Skill that will be used by other instances ("the agent B"). the agent A helps you design and refine instructions, while the agent B tests them in real tasks. This works because the agent models understand both how to write effective agent instructions and what information agents need.

**Creating a new Skill:**

1. **Complete a task without a Skill**: Work through a problem with the agent A using normal prompting. As you work, you'll naturally provide context, explain preferences, and share procedural knowledge. Notice what information you repeatedly provide.

2. **Identify the reusable pattern**: After completing the task, identify what context you provided that would be useful for similar future tasks.

   **Example**: If you worked through a BigQuery analysis, you might have provided table names, field definitions, filtering rules (like "always exclude test accounts"), and common query patterns.

3. **Ask the agent A to create a Skill**: "Create a Skill that captures this BigQuery analysis pattern we just used. Include the table schemas, naming conventions, and the rule about filtering test accounts."
...
4. **Review for conciseness**: Check that the agent A hasn't added unnecessary explanations. Ask: "Remove the explanation about what win rate means - the agent already knows that."

5. **Improve information architecture**: Ask the agent A to organize the content more effectively. For example: "Organize this so the table schema is in a separate reference file. We might add more tables later."

6. **Test on similar tasks**: Use the Skill with the agent B (a fresh instance with the Skill loaded) on related use cases. Observe whether the agent B finds the right information, applies rules correctly, and handles the task successfully.

7. **Iterate based on observation**: If the agent B struggles or misses something, return to the agent A with specifics: "When the agent used this Skill, it forgot to filter by date for Q4. Should we add a section about date filtering patterns?"

**Iterating on existing Skills:**

The same hierarchical pattern continues when improving Skills. You alternate between:

* **Working with the agent A** (the expert who helps refine the Skill)
* **Testing with the agent B** (the agent using the Skill to perform real work)
* **Observing the agent B's behavior** and bringing insights back to the agent A

1. **Use the Skill in real workflows**: Give the agent B (with the Skill loaded) actual tasks, not test scenarios

2. **Observe the agent B's behavior**: Note where it struggles, succeeds, or makes unexpected choices

   **Example observation**: "When I asked the agent B for a regional sales report, it wrote the query but forgot to filter out test accounts, even though the Skill mentions this rule."

3. **Return to the agent A for improvements**: Share the current SKILL.md and describe what you observed. Ask: "I noticed the agent B forgot to filter test accounts when I asked for a regional report. The Skill mentions filtering, but maybe it's not prominent enough?"

4. **Review the agent A's suggestions**: the agent A might suggest reorganizing to make rules more prominent, using stronger language like "MUST filter" instead of "always filter", or restructuring the workflow section.

5. **Apply and test changes**: Update the Skill with the agent A's refinements, then test again with the agent B on similar requests
...
**Why this approach works**: the agent A understands agent needs, you provide domain expertise, the agent B reveals gaps through real usage, and iterative refinement improves Skills based on observed behavior rather than assumptions.
...
* **opencode**: Can install packages from npm and PyPI and pull from GitHub repositories
* **External API calls**: May have restricted network access depending on configuration

List required packages in your SKILL.md and verify they're available in the tool documentation.
