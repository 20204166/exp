...
## 18. PHASE 12I-W — Windows-side prep confirmed, then a pairing attempt succeeded and self-reverted (2026-09-18, 15:35–15:42)

**Windows-side Phase 12I prep (steps 2–5) — all confirmed:**

| Check | Result |
|---|---|
| Windows version | `system-analyzer 1.6.1.5` |
| Windows runtime PID | `2508` (first instance this phase) |
| Windows listener | `0.0.0.0:27321` — matches expected stable port |
| Direct TCP `192.168.55.107:27321` | `TcpTestSucceeded: True` |

Before the 10-minute continuity window could be started and observed under my
own control, pairing activity happened (user-initiated, in parallel) and
succeeded. Recording the full sequence exactly as captured, since it's the
clearest evidence this entire investigation has produced.

### The pairing succeeded

`system-analyzer.log`, first time ever showing real pairing-lifecycle detail:

```
15:37:38.020 INFO maintenance.ui.window_discovery: Pair request received from node-983364764039f9e6625e687273710909
             (identity: cf1e:352f:367e:4111:a9c6:696c:2871:af2d:f65d:e0e9:6b69:3dc6:4c02:831d:4008:6a04,
              transport: 1a7b:7cc8:e529:c7f7:2e92:4ce7:51d8:ce0f:54bf:b526:30ca:5fe0:263d:cd5a:c340:adc7)
15:37:38.036 INFO maintenance.ui.window_discovery: Showing pairing dialog for node-983364764039f9e6625e687273710909
15:37:41.629 INFO maintenance.ui.window_discovery: Pairing dialog answered for node-983364764039f9e6625e687273710909: approved=True
15:37:41.642 INFO maintenance.ui.window_discovery: Pair request from node-983364764039f9e6625e687273710909 complete: approved=True
15:38:39.379 WARNING window: Background task failed: caller lacks permission
```

Note the direction: **Windows received** the pair request (this is Linux-initiated,
i.e. Linux → Windows), the requester and both fingerprints match the known Linux
identity exactly. `cluster.json` immediately after confirmed:

```json
"trusted_nodes": [{
  "node_id": "node-983364764039f9e6625e687273710909",
  "display_name": "This System",
  "hostname": "This System",
  "host": "192.168.55.107",
  "platform": "Linux",
  "capabilities": ["component_read","dashboard_read","process_review","storage_review"],
  "trusted_at": 1789742309.6637394
}]
```

**Important correction to findings #5/#12:** the persisted trust record itself
carries `"hostname": "This System"` — this is not a Windows-side display/label
bug. **The Linux side is transmitting the literal string "This System" as its
own hostname** in the pairing payload. The real fix belongs on the Linux side
(populate the actual machine hostname, e.g. `m75-node1`), not in Windows
presentation code.

### The trust then silently self-reverted within ~4 minutes

On a later screenshot of the same running instance, Nodes & Connections showed:
`"No trusted nodes yet."` with a footer message: **"Node removed from trusted
machines."** `cluster.json` confirmed: `trusted_nodes: []` again. **Zero log
lines** exist anywhere explaining the removal — the log jumps straight from the
`caller lacks permission` warning to routine discovery/NVML lines with nothing
about a revoke, a timeout, or a removal event.

### Diagnostics → Save performance capture reveals the exact mechanism

Where the log was silent, the Diagnostics export
(`observability-20260918T144215Z.json`) was not. Exact relevant contents:

```json
"most_recent_failure": "remote transport failed: timed out",

"operations": [
  {
    "key": "node:node-983364764039f9e6625e687273710909:connect",
    "last_error": "remote transport failed: timed out",
    "last_success": null
  },
  {
    "key": "node:node-983364764039f9e6625e687273710909:trust_revoke",
    "has_result": true,
    "last_error": null,
    "last_success": 1789742493.3699887
  },
  {
    "key": "node:node-983364764039f9e6625e687273710909:pair",
    "generation": 2,
    "last_error": "remote transport failed: timed out",
    "last_success": null
  }
...
"metrics": [
  {"target": "app:node:node-983364...710909:connect", "failures": 1, "successes": 0, "last_error": "RemoteTransportError"},
  {"target": "app:node:node-983364...710909:pair",     "failures": 2, "successes": 0, "last_error": "RemoteTransportError"},
  {"target": "app:node:node-983364...710909:trust_revoke", "failures": 0, "successes": 1}
]
```

**This precisely reconstructs the failure chain:**

1. Pairing handshake completes and is approved (confirmed above — real success,
   not a false positive).
2. The app then attempts to **`connect`** to the newly-trusted node — presumably
   to establish the live authenticated session/hello that the trust is for.
3. That connect attempt fails with **`RemoteTransportError` / "remote transport
   failed: timed out"**.
4. This automatically triggers **`trust_revoke`**, which itself completes
   successfully (no error) — cleanly undoing the pairing that had just succeeded.
5. A second `pair` attempt (`generation 2`) also hit the identical
   `"remote transport failed: timed out"` error.
6. None of steps 2–5 produced a single line in `system-analyzer.log`. The
   Diagnostics capture is the only reason any of this is visible at all.

**This is likely the actual first broken boundary** this whole multi-phase
investigation has been hunting for: not discovery, not TLS, not the pairing
handshake itself (all now proven working) — but the **post-pair `connect` step
timing out**, which silently self-heals by revoking the trust it just granted,
leaving no trace for a user to diagnose. Whether this connect timeout is caused
by the VPN-address-selection hypothesis from earlier phases (candidate address
order) or a separate issue is **not yet proven** — only that this specific
operation, at this specific stage, is where the chain actually breaks.

**Recommendation for the Linux side:** (1) log `trust_revoke` and the triggering
`connect`/`pair` failure at INFO/WARNING level, matching the detail already
present for the pair-request lifecycle; (2) surface "remote transport failed:
timed out" (or a summary of it) directly in the Windows UI instead of silently
reverting to "no trusted nodes" with no explanation; (3) investigate why `connect`
times out immediately after a `pair` that just succeeded, given direct TCP to
`192.168.55.107:27321` tests successfully outside the app.

No source code was changed on Windows. The 10-minute discovery-continuity
window from the Phase 12I plan was not formally completed under controlled
observation (this activity happened first) — that measurement is still
outstanding if a clean run is wanted.

### Addendum — corrected precise timeline (deeper review, same evening)

The observability JSON's timestamps are raw Unix epoch seconds (confirmed
correct — Python's own `time.time()` on this machine agrees with the OS clock).
**Caution for anyone converting them by hand on this machine:** naive `.NET`
conversions (`[datetime]::AddSeconds(epoch).ToLocalTime()`) silently fail to
apply British Summer Time on this install (`Get-TimeZone` → "GMT Standard Time",
`SupportsDaylightSavingTime: True`) and land exactly **1 hour behind** true local
time — while `Get-Date`, the log file's own timestamps, and Python's
`time.localtime()` all correctly show BST (`is_dst: 1`). This is a quirk of the
conversion method, not a bug in the app or the OS clock. Using
`[System.TimeZoneInfo]::ConvertTimeFromUtc(..., "GMT Standard Time")` instead
gives conversions that line up exactly with independent evidence (e.g. the
diagnostics file's own `LastWriteTime` on disk).

**Corrected, precise sequence:**

| Local time | Event |
|---|---|
| 15:37:38 | Pair request received from Linux (log) |
| 15:37:41 | Pairing approved (log) |
| **15:38:29** | Trust persisted to `cluster.json` (`trusted_at`) — 48s after approval |
| *(app closed, relaunched with dev-mode flag per user request)* | |
| **15:41:10** | New process (PID 9196) session start — matches its own first discovery log line (15:41:12) almost exactly |
| **15:41:33** | `trust_revoke` completes successfully — 23s after the new process started |
| 15:41:36 | Routine CPU component refresh (unrelated, just for cross-check) |
| 15:42:15 | Diagnostics capture saved (this analysis) |

**This refines the finding meaningfully.** The trust revoke did **not** happen
in the same session/process as the original successful pairing — it happened
23 seconds into the *next* process's startup, after that fresh process read the
already-trusted peer from `cluster.json` and attempted to `connect` to it. That
connect attempt timed out, and the app immediately (single attempt, no visible
retry/backoff) revoked the trust it had persisted from the prior session.

**Restated precisely:** trust survives being written to disk and survives an app
restart in principle (it was still there when the new process read it) — but the
**first startup reconnection attempt** to a previously-trusted peer, if it times
out even once, silently destroys that trust with no retry and no user-visible
explanation. This is a slightly different and more specific boundary than
"pairing self-reverts" — it's specifically the **startup/reconnect path for
already-trusted nodes** that has no tolerance for a single slow or failed
connection attempt.

**Corroboration from the app's own documentation:** Settings → Help & Guide →
"Pairing & Trust" describes the intended model explicitly: *discover → press
Pair and confirm the identity fingerprint → the other machine's user sees an
incoming pairing prompt and chooses Accept/Reject → "Only after Accept does a
trusted peer relationship exist."* It further distinguishes **Discovered**,
**Paired/Trusted**, and **Authorized** as three separate, deliberate states.
Nothing in this documented model describes trust as conditional on a subsequent
successful `connect`, nor as something that can be silently revoked by a timeout
with no user action. The auto-revoke-on-connect-timeout behavior found above is
**not part of the app's own documented trust model** — supporting that this is
unintended behavior, not a documented safety feature working as designed.

**Also worth a second look later (not chased further this session):** the
`"caller lacks permission"` warning at 15:38:39 fell only ~10 seconds after
`trusted_at` (15:38:29) in the *original* pairing session — i.e. a background
task appears to have tried to use the newly granted capabilities almost
immediately after trust was persisted, and hit a permission check that may not
yet have caught up with the fresh grant. Whether this is the same underlying
race as the connect-timeout/revoke sequence above, or a separate symptom, is
unconfirmed — flagged as a lead, not a conclusion.

---

## 19. UNIFYING FINDING — "Target did not provision the peer grant" IS a RemoteTransportError timeout, not a semantic rejection (2026-09-18, ~15:57)

**This consolidates every pairing failure recorded across this entire
investigation (1.6.0.5 through 1.6.1.5) into one root cause.**

### Controlled reproduction, this session

With the confirmed-correct Linux peer discovered (Node ID
`node-983364764039f9e6625e687273710909`, port `27321`, matching TLS fingerprint),
performed a fresh **Windows-initiated** pair attempt end to end, watching closely:

1. **15:57:21** — clicked **Pair** on the discovered peer card.
2. Card-level confirmation popup appeared: *"Review this node before pairing."*
3. **15:57:37** — clicked **Pair** on that popup.
4. A full **"Confirm peer fingerprint"** dialog appeared — *"Pair This System?"*,
   showing the complete, untruncated Stable node ID, Identity fingerprint, and
   TLS fingerprint, all matching the authoritative Linux identity exactly, with
   **Yes**/**No** buttons. (This is the proper initiator-side confirmation step —
   working as documented in the app's own "Pairing & Trust" help topic.)
5. **15:57:55** — clicked **Yes**.
6. Result, within ~2 seconds: **"Target did not provision the peer grant"** — the
   same text seen in every prior failed session.
7. `system-analyzer.log`: **zero new lines** for any of steps 1–6.
8. `cluster.json`: unchanged, still empty (`trusted_nodes: []`, `peer_grants: []`)
   — this attempt never got as far as persisting anything, unlike the earlier
   Linux-initiated success.

### What the Diagnostics capture actually shows for this exact attempt

Saved immediately after (`observability-20260918T145828Z.json`):
...
"metrics": [{
  ...
  "failures": 1,
  "successes": 0,
  "last_error": "RemoteTransportError",
  "samples": [0.2621430999934091]
}]
```

**"Target did not provision the peer grant" is the generic UI wording for a
`RemoteTransportError` transport-layer timeout — not a semantic
rejection/denial from the Linux peer.** The wording has, throughout this whole
investigation, strongly implied the *target* actively declined to provision
something (an application-level decision). The diagnostics evidence says
otherwise: the underlying failure is the same transport timeout class
(`RemoteTransportError` / "remote transport failed: timed out") seen in finding
#18's post-pair `connect` failure, just occurring here at the `pair` RPC itself
instead of the follow-up `connect`.

### This reframes the whole investigation's pairing findings

| Direction | Outcome | Underlying error |
|---|---|---|
| Linux → Windows (Linux initiates) | `pair` RPC succeeds, trust persisted | none — this path works |
| ...then Windows' own post-pair `connect` to Linux | fails | `RemoteTransportError` (finding #18) |
| Windows → Linux (Windows initiates) | `pair` RPC fails immediately | `RemoteTransportError` (this finding) — same class |

Every Windows-initiated pairing failure across every wheel version tested
(1.6.0.5, 1.6.0.7, 1.6.1.0, 1.6.1.4, 1.6.1.5) showing "Target did not provision
the peer grant" is now understood to most likely be this same transport timeout,
not a peer-side decision. This gives strong (though not yet packet-level-proven)
support to the VPN-address-selection hypothesis raised in earlier phases
(PHASE 12F Part E): plain `Test-NetConnection 192.168.55.107:27321` succeeds
every time because it's hardcoded to the correct LAN address, but the
application's own `pair`/`connect` RPC calls may be resolving or attempting a
**different** address under the hood (one of Linux's VPN addresses,
`10.2.0.2` or `100.85.0.1`) — which would time out exactly like this, while raw
TCP to the LAN address keeps testing fine. **Not proven without Linux-side
evidence of which address the incoming connection attempt (or lack thereof)
targeted** — but this is now the single strongest, most specific lead this
entire investigation has produced, and it unifies nearly every network/pairing
finding recorded (#3, #4, #5, #12, #13, #15, #16, #18) under one plausible root
cause.

**Recommendation for the Linux side:** (1) log the actual target address/port
the `pair` and `connect` RPCs attempt, on both success and
`RemoteTransportError` paths; (2) stop surfacing `RemoteTransportError` as
"Target did not provision the peer grant" in the UI — the current wording
actively misdirects debugging toward peer-side/application logic when the
evidence points at transport/address-selection; (3) if multiple candidate
addresses exist for a discovered peer, log which one was tried and enforce
LAN-address preference (already the stated intent of the 1.6.1.3
`_lan_address_key` change per this doc's PHASE 12F prep notes) for **all**
outbound RPCs, not just initial discovery display.

No source code was changed on Windows. Both diagnostics captures from this
session (`observability-20260918T144215Z.json`,
`observability-20260918T145828Z.json`) are preserved on disk as evidence.

### Addendum — 3 more captures, 4 attempts now consistently clustered at ~0.26s

User saved three further captures in quick succession:
`observability-20260918T145946Z.json`, `observability-20260918T145947Z.json`
(1 second apart), and `observability-20260918T150017Z.json`. By the last one,
the `pair` target's sample set had grown to **4 failed attempts**, all with the
same `RemoteTransportError`:

```json
"target": "app:node:node-983364764039f9e6625e687273710909:pair"
"failures": 4, "successes": 0, "last_error": "RemoteTransportError"
"samples": [0.2621430999934091, 0.2622523000027286, 0.2597948999973596, 0.26011290000315057]
```

**All four durations fall within ~2.5ms of each other (~0.26s each).** No
`connect` target appears in these later captures — only repeated `pair`
attempts. `most_recent_failure` remains `"remote transport failed: timed out"`
throughout; `trusted_nodes` stays empty.

**This tight, near-identical clustering is itself informative.** A genuine
network timeout waiting for an unresponsive-but-routable host would typically
show more variance and take considerably longer — Windows' default TCP connect
timeout (SYN retry exhaustion) is on the order of ~20 seconds, not ~0.26s. A
fast, *consistent, sub-second* failure repeated identically across four separate
attempts is far more characteristic of an **immediate OS-level rejection** (e.g.
"no route to host" / connection refused) than of an actual multi-second network
timeout being patiently waited out. That is exactly the signature you'd expect
if the RPC is attempting to reach an address Windows has no route to at all —
such as one of Linux's VPN-only addresses (`10.2.0.2`, `100.85.0.1`) — which
would fail near-instantly at the OS level, rather than a slow timeout against
the correct-but-somehow-unresponsive LAN address. This further strengthens
(without yet proving) the address-selection hypothesis in the main finding
above: the label "timed out" in the app's own error text may itself be a
slight mischaracterization of what's actually a fast connection-refused/
unreachable failure.

All five diagnostics captures from this session are preserved on disk as
evidence: `observability-20260918T144215Z.json`,
`observability-20260918T145828Z.json`, `observability-20260918T145946Z.json`,
`observability-20260918T145947Z.json`, `observability-20260918T150017Z.json`.

---

## 20. PHASE 12J-W — wheel 1.6.1.6, transport regression + directional asymmetry confirmed live (2026-09-18, 17:40–17:47)

**Windows-side prep — all confirmed:**

| Check | Result |
|---|---|
| Windows version | `system-analyzer 1.6.1.6` |
| First instance PID | `8136`, listener `0.0.0.0:27321`, direct TCP to `192.168.55.107:27321` → `TcpTestSucceeded: True` at that point |
| Discovery | **2 peers now shown** — see below |

### New, separate observation: a second bogus self-discovery entry

Nodes & Connections showed **two** "This System" cards simultaneously:
1. The genuine Linux peer — Node ID `node-983364764039f9e6625e687273710909`,
   port `27321`, `Pairing: discovered` (clean, no prior failure baked in).
2. A **second, different entry**: same display name "This System", but
   **Node ID `local`** and **port `0`**. This looks like a new self-discovery
   leak in 1.6.1.6 — the local machine appearing to discover itself as a
   separate, bogus peer with placeholder identity (`local`) and placeholder
   port (`0`). Not investigated further (out of scope for this pass, and never
   selected for pairing), but flagged as a new, distinct defect from the
   existing "This System" mislabel on the *real* Linux peer (finding #5/#12).
   **Not present in earlier sessions' screenshots** — appears to be new in
   1.6.1.6, possibly related to the Fix 2 (address-selection) or Fix 4 (auth
   classification) changes touching discovery/candidate code.

### Transport regressed mid-session

After closing that first instance and relaunching (PID `6612`, per your
request, with the developer-mode flag set so Diagnostics capture would be
available) to redo the test cleanly:

```
Get-NetTCPConnection -State Listen | Where OwningProcess -eq 6612
  0.0.0.0  27321          <- listener still correct
...
Confirmed **twice** (not transient). This is a genuine regression from the
**same machine pair** minutes earlier (PID 8136 tested `True` at ~17:40).
Per the Phase 12J-W plan's Step 5 stop rule, **no Windows-initiated Pair attempt
was made** while transport was down.

### But a Linux-initiated pair succeeded anyway, during the same window

```
17:47:03.506 INFO maintenance.ui.window_discovery: Pair request received from node-983364764039f9e6625e687273710909
             (identity: cf1e:352f:...:6a04, transport: 1a7b:7cc8:...:adc7)
17:47:03.522 INFO maintenance.ui.window_discovery: Showing pairing dialog for node-983364764039f9e6625e687273710909
17:47:05.781 INFO maintenance.ui.window_discovery: Pairing dialog answered for node-983364764039f9e6625e687273710909: approved=True
17:47:05.781 INFO maintenance.ui.window_discovery: Pair request from node-983364764039f9e6625e687273710909 complete: approved=True
```

Same exact pattern as the original success in finding #18 — but note the
approval this time took ~2.3s (17:47:03.5 → 17:47:05.8), versus ~3.6s the first
time; both fast, human-plausible approval times, consistent.

**Confirms the directional asymmetry precisely:** Linux→Windows connections
(inbound to Windows) keep succeeding even while Windows→Linux transport
(outbound from Windows, what `Test-NetConnection` measures and what a
Windows-initiated Pair needs) is actively broken at the same moment. This is
strong evidence the two directions use genuinely different network paths or
are affected by genuinely different conditions — not a single symmetric
"the network is down" explanation.

### Trust persisted to a *different* field this time — worth flagging as its own inconsistency

```json
"trusted_nodes": [],
"peer_grants": ["node-983364764039f9e6625e687273710909"]
```

The **first** successful Linux-initiated pair (finding #18) persisted to
`trusted_nodes`. **This second, functionally-identical success persisted to
`peer_grants` instead**, with `trusted_nodes` empty. Same event type (Linux
sends pair request → Windows shows dialog → approved=True), two different
outcomes in terms of which record gets created. Not diagnosed further here —
flagged for the Linux side, since per the Phase 12J audit notes
`TrustedNodeRecord` and `PeerGrantRecord` are meant to represent distinct,
specific relationships (who trusts whom / who granted whom access), and having
the *same kind of event* land in different fields across two otherwise-identical
runs suggests either non-determinism or that these two events weren't actually
as identical as their logs make them look.

**Status:** Windows-initiated Pair was correctly never attempted this round
(transport was down, per the stop rule) — this is not yet a completed Phase
12J-W pass. The valuable evidence captured instead is the live confirmation of
the directional asymmetry and this new trust-field inconsistency. Re-run needed
once Windows→Linux transport is confirmed stable again.

No source code was changed on Windows.
