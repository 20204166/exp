...
|---|---|---|---|
| `same_cluster` | `bool \| None` | `None` | Node has an active (non-revoked) assignment in this coordinator's cluster; `None` = not verified |
| `worker_eligible` | `bool \| None` | `None` | Node holds WORKER role and is neither paused nor revoked; `None` = not verified |

Both default to `None` (unknown / not verified). For MOVABLE, `None` fails **closed** — unknown eligibility is treated as ineligible. For TARGET_BOUND and LOCAL_BOUND these fields are ignored entirely, so the `None` default causes no regression.

**MOVABLE eligibility is fail-closed by design: unknown facts are ineligible.**

**`_rejection_for` gains MOVABLE-specific checks (Phase 8.5: fail-closed `is not True`):**

```python
if request.job_class is JobClass.MOVABLE:
    if view.same_cluster is not True:
        return "not in cluster"
    if view.worker_eligible is not True:
        return "not a worker"
```

`is not True` treats both `None` (unknown) and `False` (known-ineligible) as rejected. This is the fail-closed contract: unknown eligibility is ineligible. These checks run before the universal trust/auth/online tuple.

**`placement_view_for_context()` accepts two new keyword params (Phase 8.5: default `None`):**
...
The `None` default means a view built without explicit cluster facts is MOVABLE-ineligible by default — fail-closed. The production TARGET_BOUND call site passes neither param; TARGET_BOUND never enters the MOVABLE branch (job-class check gates it), so `None` defaults cause no TARGET_BOUND regression.
...
`PlacementPolicy._rejection_for` checks `same_cluster`/`worker_eligible` only when `job_class is JobClass.MOVABLE`. TARGET_BOUND never enters that branch regardless of field values; the `None` defaults in `placement_view_for_context` cause no TARGET_BOUND regression.
...
| `test_single_machine_explicit_facts_leave_local_eligible` | local context with explicit `same_cluster=True, worker_eligible=True` → eligible |
| `test_single_machine_default_fields_fail_closed_for_movable` | local context with no cluster facts (defaults `None`) → "not in cluster" |
...
### Phase 8.5 — Fail-closed MOVABLE placement contract hardening

**Problem (Phase 8 was fail-open):** The `bool = True` defaults meant any hand-crafted `PlacementView` without explicit cluster fields appeared MOVABLE-eligible. MOVABLE eligibility must never default to yes.

**Fix:** Both fields changed to `bool | None = None`. `_rejection_for` uses `is not True` — `None` (unknown) and `False` (known-ineligible) are both rejected for MOVABLE.

**`ClusterState.is_active_coordinator` property added (`maintenance/cluster.py`):**
...
`True` only when: epoch exists AND epoch names this node as coordinator AND local role is COORDINATOR AND not paused AND not revoked. A stale former Coordinator whose epoch was superseded (`coordinator_id.value != local_node_id`) returns `False`.

**Authority chain:** `cluster_state.is_active_coordinator` → `build_movable_views` → `PlacementPolicy.choose`. Worker, Subcoordinator, and stale Coordinator cannot initiate MOVABLE placement. `build_movable_views` docstring documents this as a caller responsibility.

**`build_movable_views` docstring updated** with explicit authority gate: callers must check `cluster_state.is_active_coordinator` before invoking.

**Additional tests added (`tests/test_placement.py`):**

| Test | What it proves |
|---|---|
| `test_movable_with_unknown_cluster_membership_fails_closed` | `same_cluster=None` (omitted) → "not in cluster" |
| `test_movable_with_unknown_worker_eligibility_fails_closed` | `worker_eligible=None` (omitted) → "not a worker" |
| `test_target_bound_with_none_cluster_fields_is_not_rejected` | TARGET_BOUND with `None` fields → no regression |

**`CoordinatorAuthorityTests` added (`tests/test_cluster_roles_persistence.py`, 8 tests):**

| Test | What it proves |
|---|---|
| `test_active_coordinator_returns_true` | Standard Coordinator → True |
| `test_worker_only_returns_false` | WORKER only, no COORDINATOR role → False |
| `test_subcoordinator_returns_false` | SUBCOORDINATOR only → False |
| `test_no_epoch_returns_false` | COORDINATOR role but no epoch → False |
| `test_stale_former_coordinator_returns_false` | Epoch belongs to a different node → False |
| `test_paused_coordinator_returns_false` | paused=True → False |
| `test_revoked_coordinator_returns_false` | revoked=True → False |
| `test_promoted_new_coordinator_returns_true` | New epoch with local node as coordinator → True |

### Status matrix updates

| Entry | Previous status | New status |
|---|---|---|
| `placement MOVABLE` | `MISSING WIRING` / dormant | `FUTURE SEAM` — `PlacementPolicy` now enforces cluster/role checks (fail-closed); `build_movable_views` prepares the view projection; `is_active_coordinator` gates orchestration authority; no production MOVABLE caller yet |
...
## §65 — Phase 12A: Cross-Platform TLS Generation Repair

**Date:** 2026-09-17 | **Commit:** (see git log) | **Version:** 1.6.0.5 (planned)

### Windows Pairing Failure — Root Cause

**Failure boundary:** B — TLS credential generation.

`maintenance/remote_security.py` previously called
`subprocess.run(["openssl", "req", "-x509", ...], check=True, stderr=subprocess.DEVNULL)`
to generate the per-node self-signed certificate and private key.

On Windows, `openssl.exe` is not available in PATH in a standard Python or packaged
install. With `check=True` and `stderr=subprocess.DEVNULL`, the failure surfaces as
an unhandled `FileNotFoundError` that is swallowed without a user-visible message,
crashing TLS material generation silently. Without the certificate, the `RemoteSocketServer`
cannot start, and the pairing architecture never reaches the network — every downstream
stage (discovery, TCP connect, TLS handshake, pair request) is unreachable.

**What was NOT the problem:** the pairing protocol, transaction ordering, Tk prompts,
HMAC authentication, mDNS discovery, socket binding, or firewall were not the root
cause. All of those layers depend on the TLS identity existing first.

### Fix

`_generate_self_signed()` replaces the `subprocess.run` call. It uses the `cryptography`
Python library (added as an explicit runtime dependency: `cryptography>=3.0`) to
generate an RSA-2048 private key and a self-signed X.509 certificate with the same
parameters as the previous openssl invocation:

- 2048-bit RSA key, exponent 65537
- `CN=<node_id>` subject and issuer
- 3650-day validity
- SHA-256 signature
- PEM encoding for both key and certificate

The output files (`peer-tls.crt`, `peer-tls.key`) are byte-for-byte compatible with
the previous openssl output. The `ssl.PEM_cert_to_DER_cert` conversion for fingerprint
computation and `ssl.SSLContext.load_cert_chain` loading are unchanged.

### Existing Certificate Compatibility

`ensure_tls_material` only generates when both files are absent. Existing Linux/macOS
installations that already have `peer-tls.crt` and `peer-tls.key` generated by the
old openssl path are reloaded unchanged. No re-pairing is required.

### Windows `chmod` Limitation

`os.chmod(0o600)` on Windows sets the read-only bit but does NOT restrict file access
to the current user via Windows ACLs. The private key is stored in the app's private
config area (`APPDATA\system-analyzer\`) but is not ICACLs-hardened. This is documented
as a platform limitation. No Windows ACL hardening is applied automatically.

### Cross-Platform Status Table

| Platform | TLS generation | Listener | Discovery | Pairing | Physical evidence |
|---|---|---|---|---|---|
| Linux | VERIFIED PHYSICAL (pre-12A) | VERIFIED PHYSICAL | VERIFIED PHYSICAL | VERIFIED PHYSICAL | Real two-node (all phases) |
| Windows | REAL-SOCKET ONLY (post-12A fix) | NOT VERIFIED PHYSICAL | NOT VERIFIED PHYSICAL | NOT VERIFIED PHYSICAL | Pending real hardware run |
| macOS | UNIT ONLY | UNIT ONLY | NOT VERIFIED | NOT VERIFIED | No physical run |

### Tests Added (Phase 12A)

- `test_generation_does_not_require_openssl_on_path` — proves `subprocess.run` failure
  does not prevent generation
- `test_generated_certificate_is_valid_pem_loadable_by_ssl` — verifies PEM output is
  consumable by Python `ssl`
- `test_fingerprint_computed_from_der_bytes_not_pem_text_encoding` — verifies CRLF in
  PEM text does not change fingerprint (DER-canonical)
- `test_partial_state_missing_key_triggers_regeneration` — missing key → full regenerate
- `test_partial_state_missing_cert_triggers_regeneration` — missing cert → full regenerate

### Removed Test

`test_tls_generation_forwards_no_window_creation_flag` — tested the subprocess
`CREATE_NO_WINDOW` flag; no longer applicable after subprocess removal.

### NOT VERIFIED Items (Phase 12A)

The following require physical Windows hardware to verify:

- Windows Firewall behavior (inbound TCP for Python / packaged exe)
- mDNS advertisement on Windows (Zeroconf `_system-analyzer._tcp.local.`)
- Real Linux → Windows Pair
- Real Windows → Linux Pair
- Windows TCP listener reachability from LAN peer
...
10. **COORDINATOR VIEW**: Active non-revoked `role_assignments` are authoritative for a Coordinator's membership topology. They answer: which Workers belong to my cluster, what roles do they hold, is an assignment revoked? `coordinator_epoch` plays no special role on the Coordinator side.
10b. **WORKER VIEW**: A joined Worker's persisted state authoritatively provides its `cluster_id`, its own `local_role`, and `coordinator_epoch.coordinator_id`. A joined Worker may identify "this peer is my Coordinator" from `coordinator_epoch.coordinator_id` even if it does not persist a separate active `role_assignment` for the Coordinator. `coordinator_epoch` identifies ONLY the current Coordinator relationship — it must NOT be used to fabricate arbitrary Workers, full topology, or membership for unrelated trusted peers.
11. **Membership precedence for a peer NodeId** (in order, first match wins): (1) Active non-revoked `role_assignment` exists → assignment wins; (2) Explicit `role_assignment` with `revoked=True` exists → revoked wins, epoch fallback is suppressed; (3) No assignment AND peer NodeId equals `coordinator_epoch.coordinator_id` on a joined Worker → peer is the Coordinator; (4) Otherwise → no membership may be inferred.
11b. **Three axes must remain separate**: TRUST (pairing/trusted records/peer grants) · CLUSTER (Coordinator role_assignments / Worker joined state + Coordinator identity) · CONNECTION (Online/Offline). Do not collapse these.
...
18. After JOIN, the Coordinator card in Nodes & Connections (TrustedNodeSpec) must show `role="coordinator"`, `is_cluster_member=True`, meta text "Trusted · Coordinator · Cluster member · Online/Offline".
...
26. `TrustedNodeSpec._trusted_meta_text()` reads `is_cluster_member` and `role` to produce "Trusted · Coordinator · Cluster member · Online" vs "Trusted · Not in cluster · Online". The explicit "Cluster member" label is rendered for every active member regardless of role.
27. `ClusterNodeSpec._meta_text()` reads `is_cluster_member` and `role` to produce "Trusted · Online · Coordinator · Cluster member" vs "Trusted · Online · Not in cluster". The explicit "Cluster member" label is rendered for every active member, including the local node's own row.
...
§67 implemented the epoch-based coordinator fallback so `is_cluster_member=True` reached the specs correctly. §68 makes that membership **visible** by rendering the literal "Cluster member" label in both presentation layers, and corrects the canonical model description to separate the Coordinator view from the Worker view.

### Rendering changes

| Layer | File | Before | After |
|---|---|---|---|
| Nodes & Connections `_trusted_meta_text()` | `maintenance/ui/nodes_connections.py:708` | `Trusted · Coordinator · Online` | `Trusted · Coordinator · Cluster member · Online` |
| All Systems `_meta_text()` | `maintenance/ui/cluster_page.py:417` | `Trusted · Online · Coordinator` | `Trusted · Online · Coordinator · Cluster member` |

The marker appears for every active member (local or remote, coordinator or worker). It is absent for non-members ("Not in cluster") and for discovered/untrusted candidates.

### Membership marker rule

| Node | Visible marker |
|---|---|
| Local active Coordinator | Yes |
| Local joined Worker | Yes |
| Remote current Coordinator (from Worker) | Yes |
| Remote active Worker (from Coordinator) | Yes |
| Active member while Offline | Yes |
| Merely discovered peer | No |
| Paired non-member | No |
| Trusted non-member | No |
| Revoked assignment | No |

### Physical topology for acceptance test

| Node | ID |
|---|---|
| Coordinator (Linux, m75-node1) | `node-983364764039f9e6625e687273710909` |
| Worker (Windows, DESKTOP-0C2C5H3) | `node-b31a0913e76db7c60cb7c6ec82313660` |

### Physical acceptance contract (Worker UX)

**BEFORE JOIN:**
`Trusted · Not in cluster · Online`

**AFTER JOIN:**
`Trusted · Coordinator · Cluster member · Online`
Primary action: Remove Connection (no Pair, no Join)

**COORDINATOR OFFLINE:**
`Trusted · Coordinator · Cluster member · Offline`
(Pair remains hidden)

**ALL SYSTEMS — WORKER MACHINE:**
```
DESKTOP-0C2C5H3   Worker · Cluster member
m75-node1         Coordinator · Cluster member · Online
```

**ALL SYSTEMS — COORDINATOR MACHINE:**
```
m75-node1         Coordinator · Cluster member
DESKTOP-0C2C5H3   Worker · Cluster member · Online
```

### Automated tests added/updated

`tests/test_nodes_connections_page.py` — `NAndCMembershipLabelTests`:
- `test_case_c_paired_joined_online_shows_worker` — added `assertIn("Cluster member")`
- `test_case_d_paired_joined_offline_shows_worker_offline` — added `assertIn("Cluster member")`
- `test_case_f_coordinator_peer_online_shows_coordinator` — added `assertIn("Cluster member")`
- `test_case_f_coordinator_peer_offline_shows_coordinator` — added `assertIn("Cluster member")`

`tests/test_cluster_page.py` — `AllSystemsMetaTextMembershipTests`:
- `test_active_worker_member_shows_worker` — added `assertIn("Cluster member")`
- `test_coordinator_member_shows_coordinator` — added `assertIn("Cluster member")`
- `test_local_node_always_shows_role_not_not_in_cluster` — added `assertIn("Cluster member")`

### What was NOT changed

- Pair protocol, pairing ceremony, trust persistence — unchanged
- Join protocol, `_apply_cluster_join`, `consume_invite` — unchanged
- `coordinator_epoch` propagation — unchanged
- Discovery, TLS, HMAC, authorization — unchanged
- Share My Dashboard — unchanged
- Coordinator-side role_assignments logic — unchanged
- `node_specs.py` epoch fallback from §67 — unchanged (still needed)
